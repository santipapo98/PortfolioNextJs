...
function WorkExperience({experiences}: Props) {
  return (
    ...
    className='h-screen flex relative overflow-hidden flex-col text-left md:flex-row max-w-full md:px-10 md:p-0 justify-evenly ms-auto items-center'>
        ...
        <div className='w-full flex items-center space-x-5 overflow-x-scroll p-3 md:p-10 snap-x snap-mandatory scrollbar'>
          {experiences?.map((experience, index) => {
            return (
              <ExperienceCard key={experience._id} experience={experience} scrollRight={index+1 === experiences.length ? false: true}/>
            )
          })}
        </div>
    </motion.div>
